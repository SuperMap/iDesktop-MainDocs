...
程序为地图提供了几种预设的颜色模式，可设置当前地图窗口中所有矢量数据集的颜色模式，提供的颜色模式有默认色彩模式、黑白模式、灰度模式、黑白反色模式、黑白反色，其他颜色不变五种，设置后可即时预览地图的输出效果。
