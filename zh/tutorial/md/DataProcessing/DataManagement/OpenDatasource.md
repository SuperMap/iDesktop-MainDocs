...
型数据源三类。有关各类型数据源的特点和适用场景，请参见[数据源及数据引擎类型](EngineType)。
...
4 | **MySQL** | × | √ | √ | √ | × | 5.6.16及以上版本 |远程服务端必须保证本机有访问权限，如果没有权限可以参考语句grant all privileges on *.* to user@'%'identified by pwd; flush privileges,其中%可以换成具体的IP。 |    
 5|**[PostgreSQL](../../TechDocument/PostgreSQLUseGuide/PostgreSQLGuide)** |× | √ | √ | √ | √ | 9.6及以上版本 |远程服务端必须保证本机有访问权限，如果没有权限，到服务端数据存储路径下修改/data/pg_hba.conf文件添加host all all 0.0.0.0/0 trust，重启服务。  
...
13 | **[MongoDB](../../TechDocument/MongoDBDatabaseGuide)** | × | √ | √ |√ | × | 2.4以上版本 | 远程服务端必须保证本机有访问权限。  
